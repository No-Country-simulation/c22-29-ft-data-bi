...
inventario_df = pd.concat([sub_df1, sub_df2, sub_df3, sub_df4], ignore_index=True)

# Eliminar duplicados solo por 'VendorNumber'
inventario_df = inventario_df.drop_duplicates(subset=['InventoryId'], keep='first')
```
...
```{python}
columns_to_inventary = ['InventoryId', 'Store', 'City', 'Brand', 'Description', 'Size']
...
city_inventario_df = city_inventario_df.drop_duplicates(subset=['InventoryId'], keep='first')
```
